class ChannelParticipationsController < ApplicationController
  ...
  def update
    ...
    if (@channel_participation.is_owner && !user_is_admin_owner? && !user_is_current_channel_participant?(params[:id]))
      respond_to do |format|
        format.html
        format.json {render json: {error_text: "not_authorized"}, status: :unauthorized}
      end
    ...
    channel_id = @channel_participation.channel.id;
    if (user_is_admin_owner? || user_is_channel_owner?(channel_id) || user_is_channel_admin?(channel_id))
  ...
  def destroy
    ...
    if (user_is_current_channel_participant?(params[:id]) || user_is_admin_owner? || user_is_channel_owner?(channel_id) || user_is_channel_admin?(channel_id))
