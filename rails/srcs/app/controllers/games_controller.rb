...
class AskForGamesController < ApplicationController
  ...
  def create
	...
	#----------- Variable utils ------------------- # enlever les vérif ? ex if (guild_a != nil) et les init ex: war_a = nil
	json_render = {}
	# USER A
	user = User.find(params[:from_user_id].to_i)
	guild_a = user.guild_participations.first.guild #can bug if guild_participations is nil ?
	war_a = nil
	if (guild_a != nil)
