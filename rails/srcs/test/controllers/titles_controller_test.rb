...
class TitlesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create title" do
    assert_difference('Title.count') do
      post titles_url, params: { title: { name: @title.name, status: @title.status, tournament_id: @title.tournament_id, user_id: @title.user_id } }
    end
  ...
  test "should update title" do
    patch title_url(@title), params: { title: { name: @title.name, status: @title.status, tournament_id: @title.tournament_id, user_id: @title.user_id } }
